...
后续修复与新验证见 [描述符表连续性](hvm-descriptor-continuity.md)。上述原始
EPT 实验的二进制和引导参数保持原样，新的结果单独存档。

- 已验证的是单个 4 KiB WB RAM 页；CLI 提供填充页，IOCTL 可传完整影子页内容。
- 原始实验使用 `nosmp hpet=disable rdinit=/bin/sh`。描述符修复后的追加实验已在
  不含这三个参数的文本引导中走完正常 `/init`，并重新验证换页；来宾仍为 1 vCPU，
  时钟事件设备为 `lapic-deadline`。多核启动和 HPET clockevent 的直接回归仍未完成。
- 描述符恢复补丁后的三次常驻中 `vmrun stop` 均未触发旧的三重故障；尚不能把
  该结果推广为所有拆除方式都安全。常规夹具脚本仍保留先停常驻的顺序，整套部署
  保留 HVM TARGET 重启流程。
- `0xA` 转储中的首个异常与准备报告 GDT 损坏的路径已有证据，三处遗漏的表寄存器
  恢复已经修复并经双核回读验证；长期消除效果仍待观察。
