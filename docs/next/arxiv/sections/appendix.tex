...
\end{center}
The region cohort ran later builds of the same driver, carrying the version~4
lease. Their identity is taken from the file the service actually loaded
(\code{ImagePath} \path{\??\C:\ksword\KswordARK.sys}), not from a build output
directory, because an equal build-output hash proves only that a copy was
faithful. The reachability sweeps, the above-1\,GiB publish/stage/remove
sequence and the 1\,GiB refusals ran on
\begin{center}\footnotesize
\code{9b7771ca1868177fb9f490dce736b2a1756877e4dc450a97a7a769cd5fcecba4},
\end{center}
and the region recheck was subsequently moved into the portable planner, which
is a code change and therefore a different binary. Two were produced from those
sources and they are not interchangeable: \code{088c44a0\ldots}, from
\code{msbuild /t:Build}, under which the descendant's VM entry failed in every
run, and the clean rebuild that replaced it, which is the binary the revocation
and post-refactor reachability results ran on:
\begin{center}\footnotesize
\code{ea6965f0569050eede9f3a2917769c01231146bab18bc4a5f6d0097dfad60b4b}.
\end{center}
The controller is unchanged across both, at
\begin{center}\footnotesize
\code{cacbb90e5b1a29d7ce98bc87398838d1f924f9a5cb1a5214b640db129ce64e88}.
\end{center}
No measurement is carried across those two binaries.
...
The large-leaf results are \path{docs/next/paper-data/20260916-large-leaf/}.
\path{admission-probe.json} holds the coarse-source attempts,
\path{published.json} the both-rules comparison,
\path{stage-and-uniformity.json} the uniformity survey and the staging sequence,
\path{decoupling.json} the idle digest samples, and \path{inner-hyperv.json}
the inner Hyper-V admission attempts with the descendant's state. Each carries
the driver and controller hashes, the live EPT12 pointer, and the raw JSON of
every request and its removal.

The workload and reachability results are
\path{docs/next/paper-data/20260916-guest-workload/}. The \path{extent-*.json}
files are guest-physical sweeps of the same descendant under six configurations
--- 768\,MiB and 4096\,MiB, idle and after a workload, with and without the
settings that pin its memory and disable page sharing --- each probe recording
the status, the source leaf shift, and how many source entries the scanning rule
read. \path{large-leaf-above-1gib.json} is one publish/idle/stage/refuse/remove
sequence above the 1\,GiB line, \path{extent-4gplus.json} is the sweep that
found this descendant's bulk memory above \texttt{0x100000000}, and
\path{region-revocation.json} is the publish/snapshot/revoke/remove cycle with
every response it polled. \path{vmware-monitor-panics.txt} summarises the
descendant VMM's own log history and contains the incremental-versus-clean build
comparison. \path{docs/next/logs/Probe-Ept12Extent.ps1},
\path{docs/next/logs/Measure-LargeLeafAboveOneGiB.ps1},
\path{docs/next/logs/Measure-RegionRevocation.ps1} and
\path{docs/next/logs/Watch-RegionDrift.ps1} are the scripts that produced them;
each requires a running descendant and cannot be replayed offline.

The portable lease-walk results are
\path{docs/next/paper-data/20260916-lease-largepage/}:
\path{ept-lease-unit.txt} is the suite's own output, and
\path{ept-lease-mutation.txt} is the mutation matrix, with a column
for the suites as committed and a column for the current ones.
\path{tools/hvm_paper/Test-EptLeaseMutation.sh} regenerates both without a VM; it
fails if any mutation survives.

The no-monitor VMware baseline attempts are
\path{docs/next/paper-data/20260916-hot-insert/}. Each record states the driver
service state, the resident-monitor state, the Windows boot identity, the
observed \code{vmware-vmx} processes before and after, the launch result, and a
status of either \code{vm\_started\_guest\_boot\_unverified} or
\code{start\_rejected}. Neither is a usable baseline, and neither is reported as
one.
...
\path{docs/next/arxiv/render_pages.py} rasterizes the built PDF one PNG per page,
so the visual-review field in \path{build-validation.json} records pages someone
looked at rather than a count of overfull boxes: a table can fit the text block
and still collide with its caption, and a generated table can lose a column
without \TeX{} reporting anything.
...
\input{tables/historical-windows}
\end{table}
...
% The matrix is a tabularx and cannot break across pages, so without this the
% caption stays behind on the previous page and the table arrives on the next
% one with nothing naming it. TeX reports no warning for that.
\newpage
\begin{center}\small
\captionsetup{hypcap=false}
\captionof{table}{Evidence scope as of this draft. ``Not measured'', an observed
refusal, and a recorded attempt that produced no usable result are three distinct
statuses and are not merged. Each successful result is limited to its recorded
version.}
\label{tab:matrix}
...
Already-running VMware / TinyCore & Insert beneath whole running stack & Outside the measured scope; ownership gate retained \\
Inner Hyper-V / TinyCore, 4-by-2 & Normal boot; KSword admission & Baseline boots; VMX unavailable and admission refused \\
Inner Hyper-V descendant & EPT control under inserted KSword & Not reached; the refusal above precedes it \\
VMware / TinyCore, monitor absent & No-monitor backend baseline & Attempted twice; guest boot unverified, then start refused \\
...
2-MiB / 1-GiB source leaves & Lease capture, drift and rejection & Portable suite only; no such source leaf seen on hardware \\
Large-leaf override, coarse-source rule & Publish 2 MiB on the live descendant & Refused 9/9; VMware backs this guest at 4 KiB \\
Large-leaf override, scanning rule & Same, admitting on 512 agreeing entries & Published and removed; weaker drift detection \\
Region staging & Rewrite one page of a published region & Measured by digest; out-of-range index refused \\
Large leaf under inner Hyper-V & Publish any region & Not reachable; admission refused before any control request \\
1-GiB leaf request on hardware & Publish 1 GiB under either rule & Refused by both; 262,144 entries against a 512 bound \\
Base reachability versus configured memory & Which bases carry an override & Measured; mappings follow the workload, not memsize \\
Composition saving from one large leaf & Fills to cover a region & One composition for 512 pages; workload not aimed at the region \\
Region lease under a real source change & Snapshot the descendant while published & Revoked within 5 s, removed clean; reason is translation-changed \\
Region drift recheck, reason 4 & Revoke on a page other than the captured one & Portable suite and mutation matrix; the always-on check wins every event we can trigger \\
Incremental versus clean driver build & Same sources, \code{/t:Build} vs \code{/t:Rebuild} & Descendant VM entry failed 3/3 on the first, 0/1 on the second \\
