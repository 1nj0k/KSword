...
guest-physical page to private backing and exposes publication, invalidation,
rollback, and reclamation as separate events. Guest-side helpers select, pin,
and observe the experimental page; the page intervention itself is controlled
from the containing Windows instance.
...
ownership and execution state. That operation is not implemented here.
...
\item A version-separated evidence set reporting useful behavior together with
      ...
      bodies, paced application observations, residual CPUID/RTT overhead, and
      refused admission in an inner Hyper-V root partition.
\end{enumerate}
